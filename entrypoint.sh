...
create_dhcpd_pid_dir() {
  mkdir -p /var/run/dhcp-server
  chmod 0775 /var/cache/dhcp-server
  chown root:${DHCP_USER}
}
...
if [[ -z ${1} ]]; then
  ...
  exec "$(command -v dhcpd)" -user ${DHCP_USER} -group ${DHCP_USER} -f -4 -pf /var/run/dhcp-server/dhcpd.pid -cf /etc/dhcp/dhcpd.conf ${DHCP_INTERFACES}
else
  exec "$@"
fi
