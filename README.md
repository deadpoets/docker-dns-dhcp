...
`Dockerfile` to create a [Docker](https://www.docker.com/) container image for [BIND](https://www.isc.org/download/) DNS server, [DHCPD](https://www.isc.org/download/) bundled with the [Webmin](http://www.webmin.com/) interface.

BIND is open source software that implements the Domain Name System (DNS) protocols for the Internet. It is a reference implementation of those protocols, but it is also production-grade software, suitable for use in high-volume and high-reliability applications. ISC-DHCPD-Server is a DHCP server implementation also suitable for production
use. At some point I may migrate DHCPd services to KEA DHCPd.
...
- Support the original developer of this dockers parent image with a [donation](http://www.damagehead.com/donate/)
...
Automated builds of the image are available on [Dockerhub](https://hub.docker.com/r/deadpoets/dns-dhcp) and is the recommended method of installation.


```bash
docker pull deadpoets/dns-dhcp
```

Alternatively you can build the image yourself.

```bash
docker build -t deadpoets/dns-dhcp github.com/deadpoets/docker-dns-dhcp
```
